...
%     logpdf_invgamma = @(xx) prior_const(1,3) + prior_const(1,4) - (2.5+1)*log(xx) - 0.025./xx;
    ...
    if (theta(4) > 2)    
        ...
        newlikhood = loglik_h_HMM_v2(y, h, theta, bins, bin_midpoint);
        newlikhood = newlikhood - sum(((theta(4) + 1)/2)*log(1+(y(2:2:T).^2)./((theta(4)-2)*exp(h(2:2:T))))); 
        
        oldprior = log(hyper.N) - hyper.N*(oldtheta - 2);             
        oldconst = (0.5*T)*(gammaln((oldtheta+1)/2) - gammaln(oldtheta/2) -0.5*log((oldtheta-2)*pi));
        oldlikhood = oldlikhood - sum(((oldtheta + 1)/2)*log(1+(y(2:2:T).^2)./((oldtheta-2)*exp(h(2:2:T)))));
        
        num = newconst + newlikhood + newprior;
        den = oldconst + oldlikhood + oldprior;        
    	A = min(1,exp(num-den));
    else
        A = 0;
    end
