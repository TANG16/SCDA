function [theta, accept, oldlikhood] = update_theta_HMM_RW_eff2(y, h, theta, ...
                           bins, bin_midpoint, hyper, delta,...
                           oldlikhood)
...
%     logpdf_invgamma = @(xx) prior_const(1,3) + prior_const(1,4) - (2.5+1)*log(xx) - 0.025./xx;
    ...
    for ii = 1:3
        ...
        oldtheta = theta(ii);
%         oldlikhood = loglik_h_HMM_v2(y, h, theta, bins, bin_midpoint);
        ...
        if ((ii == 1) || ((ii == 2) && (abs(theta(ii))<1)) || ((ii == 3) && (theta(ii)>0)))
            newlikhood = loglik_h_HMM_v2(y, h, theta, bins, bin_midpoint);
...
%     oldlikhood = loglik_h_HMM_v2(y, h, theta, bins, bin_midpoint);
    ...
    % Propose a new value using a RW with uniform proposal density
    theta(4) = theta(4) + delta(4)*randn; 
    
    if (theta(4) > 2)    
        newprior = log(hyper.N) - hyper.N*(theta(4) - 2);                      
        newconst = (0.5*T)*(gammaln((theta(4)+1)/2) - gammaln(theta(4)/2) -0.5*log((theta(4)-2)*pi));
        newlikhood = loglik_h_HMM_v2(y, h, theta, bins, bin_midpoint);
        newlikhood_nu = - sum(((theta(4) + 1)/2)*log(1+(y(2:2:T).^2)./((theta(4)-2)*exp(h(2:2:T))))); 
        
        oldprior = log(hyper.N) - hyper.N*(oldtheta - 2);             
        oldconst = (0.5*T)*(gammaln((oldtheta+1)/2) - gammaln(oldtheta/2) -0.5*log((oldtheta-2)*pi));
        oldlikhood_nu = - sum(((oldtheta + 1)/2)*log(1+(y(2:2:T).^2)./((oldtheta-2)*exp(h(2:2:T)))));
        
        num = newconst + newlikhood + newlikhood_nu + newprior;
        den = oldconst + oldlikhood + oldlikhood_nu + oldprior;        
    	A = min(1,exp(num-den));
    else
        A = 0;
    end
