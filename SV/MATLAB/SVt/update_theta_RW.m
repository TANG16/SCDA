function [theta, accept] = update_theta_RW(y, h, theta, hyper, delta)
...
%     logpdf_invgamma = @(xx) prior_const(1,3) + prior_const(1,4) - (2.5+1)*log(xx) - 0.025./xx;
    ...
    if (theta(4) > 2)    
        ...
        newconst = T*(gammaln((theta(4)+1)/2) - gammaln(theta(4)/2) -0.5*log((theta(4)-2)*pi));
        newlikhood =  - sum(((theta(4) + 1)/2)*log(1+(y.^2)./((theta(4)-2)*exp(h)))); 
        
        oldconst = T*(gammaln((oldtheta+1)/2) - gammaln(oldtheta/2) -0.5*log((oldtheta-2)*pi));
        oldlikhood =  - sum(((oldtheta + 1)/2)*log(1+(y.^2)./((oldtheta-2)*exp(h))));
