function [theta, accept] = update_theta_HMM_RW_shift(y, h, theta, bins, bin_midpoint, hyper, delta, shift)
...
%     logpdf_invgamma = @(xx) prior_const(1,3) + prior_const(1,4) - (2.5+1)*log(xx) - 0.025./xx;
    ...
    for ii = 1:3
        ...
        oldtheta = theta(ii);
        oldlikhood = loglik_h_HMM_v2_shift(y, h, theta, bins, bin_midpoint, shift);
        ...
        if ((ii == 1) || ((ii == 2) && (abs(theta(ii))<1)) || ((ii == 3) && (theta(ii)>0)))
            newlikhood = loglik_h_HMM_v2_shift(y, h, theta, bins, bin_midpoint, shift);
