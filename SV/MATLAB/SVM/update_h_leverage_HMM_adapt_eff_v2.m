function [h, accept, A_sum, newloglik] = update_h_leverage_HMM_adapt_eff_v2(y, h, theta, ...
...
%             bin_midpoint = phi*(h(t-2)-mu) + s1.*mid_inv;          
            bin_midpoint = phi*(h(t-2)-mu) + rho*sigma*(y(t-2) - beta*exp(h(t-2)))./exp(h(t-2)/2) + s1.*mid_inv;          
        end
...
% m = mu + phi*bin_midpoint + rho*sigma*y(t-1)./exp(mu_bin/2);
...
bin_midpoint_next = phi*(h(t)-mu) + rho*sigma*(y(t) - beta*exp(h(t)))./exp(h(t)/2) + s1.*mid_inv;          
mu_bin_next = (mu + bin_midpoint_next);
exp_mu_bin_next = exp(mu_bin_next); 
m_next = mu + phi*bin_midpoint_next + rho*sigma*(y(t+1) - beta*exp_mu_bin_next)./exp(mu_bin_next/2);
            
%             loglik_int = Gauss_const - 0.5*(((h(t+2) - mu - phi*bin_midpoint_next).^2)/s2);
            loglik_int = Gauss_const - 0.5*(((h(t+2) - m_next).^2)/s2);
            loglik_int = loglik_int - 0.5*(log(2*pi) + mu_bin_next + ...
                ((y(t+1) - beta*exp_mu_bin_next).^2)./exp_mu_bin_next);    
            num = num + log(sum(exp(loglik_int)));
        end
...
% m = mu + phi*bin_midpoint + rho*sigma*y(t-1)./exp(mu_bin/2);
        loglik_int = Gauss_const - 0.5*(((h_old - m).^2)/s2);
        loglik_int = loglik_int - 0.5*(log(2*pi) + mu_bin + ((y(t-1) - beta*exp_mu_bin).^2)./exp_mu_bin);    
