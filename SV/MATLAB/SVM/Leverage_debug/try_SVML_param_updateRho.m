...
%     [y,h_true] = generate_SVML(theta_true,T);
    ...
    [y,h_true] = generate_SVM(theta_true,T);
...
%     delta.t = [0.35, 0.01, 0.0008, 0.065, 0.0]; % NO UPDATE FOR RHO 
    delta.t = [0.35, 0.01, 0.0008, 0.065, 0.01]; % UPDATE FOR RHO 
     
    if (arg2 > 0)
%         delta_HMM.h = 0.3;
%         delta_HMM.t = [0.05, 0.008, 0.0008, 0.07, 0.1]; %      
        delta_HMM.h = 0.4;
%         delta_HMM.t = [0.2, 0.015, 0.0015, 0.085, 0.0]; %      
        delta_HMM.t = [0.2, 0.015, 0.0015, 0.085, 0.01]; %      
    end
    if (arg3 > 0)    
%         delta_HMM_adapt.h = 0.4;
%         delta_HMM_adapt.t = [0.25, 0.009, 0.0009, 0.00, 0.1]; %  NO UPDATE FOR BETA
        delta_HMM_adapt.h = 0.4;
%         delta_HMM_adapt.t = [0.25, 0.009, 0.0009, 0.08, 0.0]; %
        delta_HMM_adapt.t = [0.25, 0.009, 0.0009, 0.08, 0.01]; %
    end
