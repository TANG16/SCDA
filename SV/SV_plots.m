...
plot(mean(H_HMM_v2_shift((5000+1):M,:),1),'m')
plot(2:2:T,mean(H_HMM_v2((5000+1):M,2:2:T),1),'b')
hold off
...
for t = 2:10
    subplot(3,3,t-1)
    hold on
    plot(H_DA(1:M,t*100)*0 + h_true(t),'r')
    plot(H_DA(1:M,t*100),'g')
    plot(H_HMM_v2(1:M,t*100),'b')
    plot(2:2:M,H_HMM_v2_shift(2:2:M,t*100),'m')
    hold off    
...
for t = 2:10
    subplot(3,3,t-1)
    autocorr(H_HMM_v2(1:M,t*100),40)
    set(gca,'ylabel',[])    
    title(['Time = ',num2str(100*t)])
%     plotTickLatex2D;%('FontSize',12);
end
...
for t = 2:10
    subplot(3,3,t-1)
    autocorr(H_HMM_v2_shift(2:2:M,t*100),40)
    set(gca,'ylabel',[])    
    title(['Time = ',num2str(100*t)])
%     plotTickLatex2D;%('FontSize',12);
end
...
for t = 2:10
    subplot(3,3,t-1)
    autocorr(H_HMM_v2_shift(1:2:M,t*100-1),40)
    set(gca,'ylabel',[])    
