model{
  ...
  for (t in 1:round(T/2)){
    ...
    Q_even[t] <- dt(y[2*t], 0, 1.0/(rho*exp(h[t])), nu)
  }
  ...
  for (t in 2:round(T/2)){ # we have T observations and T/2 imputed states
    ...
    for (i in 1:N_bin){ # bins are DEMEANED VOLATILITIES
      ...
      Q_odd[i,t] <- dt(y[2*t-1],0, 1.0/(rho*exp(bin_mid[i] + mu)), nu) # observation density for odd observations given odd states
...
#     - pnorm(bin[i], phi*(h0- mu), sigma2_star) # transition probability to h_odd[t] from h_even[t-1]
    ...
    Q_odd[i,1] = dt(y[1],0, 1.0/(rho*exp(bin_mid[i] + mu)),nu) # observation density for odd observations given odd states
