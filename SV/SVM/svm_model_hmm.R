model{
  ...
  for (t in 1:round(T/2)){
    Q_even[t] <- dnorm(y[2*t],beta*exp(h[t]), 1.0/exp(h[t])) #  observation density for even observations given even states
  }
  ...
  for (t in 2:round(T/2)){ # we have T observations and T/2 imputed states
    ...
    for (i in 1:N_bin){ # bins are DEMEANED VOLATILITIES
      ...
      # G_odd[i,t] <- pnorm(bin[i+1], phi*(h[t-1] - mu), sigma2_star) 
      # - pnorm(bin[i], phi*(h[t-1] - mu), sigma2_star) # transition probability to h_odd[t] from h_even[t-1]
      
      Q_odd[i,t] <- dnorm(y[2*t-1],beta*exp(bin_mid[i] + mu), 1.0/exp(bin_mid[i] + mu)) # observation density for odd observations given odd states
  ...
  # integration of odd
  for (i in 1:N_bin){ # bins are DEMEANED VOLATILITIES
        G_odd[i,1] = dnorm(bin_mid[i], phi*(h0- mu), sigma2_star) # transition probability to h_odd[t] from h_even[t-1]
#     G_odd[i,1] = pnorm(bin[i+1] , phi*(h0- mu), sigma2_star) 
#     - pnorm(bin[i], phi*(h0- mu), sigma2_star) # transition probability to h_odd[t] from h_even[t-1]
    Q_odd[i,1] = dnorm(y[1],beta*exp((bin_mid[i] + mu)/2), 1.0/exp(bin_mid[i] + mu)) # observation density for odd observations given odd states
