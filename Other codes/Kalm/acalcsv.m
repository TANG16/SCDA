% calculate starting values
      ...
      npars = npar1 + npara + nparp +1;                 % Includes sigma
      beta_def = [Mu; zeros(nparp,1); std(Census)];
      if exist('beta_C_C_C')
        if model1name(1)=='V'
...
% productivity
        if modelpname(1)=='V'
          ...
          for i = npar1+npara+2:npar1+npara+nparp
          ...
          for i = npar1+npara+1:npar1+npara+nparp
            fprintf('productivity %2.0f : [%5.3f] ',i-npar1-npara,exp(beta_def(i)))
            tmp = input('?');
