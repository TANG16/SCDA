...
var G1[(N_max1+1),2,T], P2[(N_max1+1),2,T], G3[(N_max3+1),2,T], P4[(N_max3+1),2,T], Q[(N_max3+1),2,T];
...
# Q - observation matrix
    ...
    for (t in 1:T){
      zeros[t] ~ dpois(PHI[t])
      PHI[t] <- -loglik[t] + C
      loglik[t] <- log(sum(sum((G1[,,t] * P2[,,t]) %*% t( G3[,,t] * P4[,,t] * Q[,,t]))))
    }
    
    for (t in 1:(T-1)){
      loglam1[t] <- log(X4[t]) + log(rho[t]) + log(phi1[t])
      loglam3[t] <- log(X2[t]) + log(phi3[t])
      for (i in 1:(N_max1-1)){
        loglam2[i+1,t] <- log(i) + log(phi2[t])
      }
    }
    ...
    for (t in 1:2){
      for (i in 0:N_max1){
        G1[i+1,1,t] <- 1/N_max1 # diffuse initialisation
        G1[i+1,2,t] <- 0 
        # for (j in 0:N_max3){
        #   P2[i+1,j+1,1] <- X2_prior[1]
        # } 
        P2[i+1,1,t] <- X2_prior[1]
        P2[i+1,2,t] <- 0
      }
      for (i in 0:N_max3){
        G3[i+1,1,t] <- 1/N_max3 # diffuse initialisation
        G3[i+1,2,t] <- 0
        P4[i+1,1,t] <- X4_prior[1]
        P4[i+1,2,t] <- 0        
        Q[i+1,1,t] <- sqrt(tauy)*exp(-0.5*tauy*pow((y[t] - (X2[t] + i + X4[t])),2))/sqrt(2*pi)
        Q[i+1,2,t] <- 0        
      }    
    } 
    ...
    for (t in 3:T){
        for (i in 0:(N_max1-1)){  # X1 (depends only on [imputed] X4)
          # G1[i+1,t] <- exp(-exp(loglam1[t-1]) + i*loglam1[t-1] - logfact(i))
          G1[i+1,1,t] <- exp(-exp(loglam1[t-2]) + i*loglam1[t-2] - logfact(i)) # 2nd order
          G1[i+1,2,t] <- 0
        } 
      G1[(N_max1+1),1,t] <- max(0,1- sum(G1[(1:N_max1),1,t]))
      G1[(N_max1+1),2,t] <- 0
      
      for (i in 0:(N_max3-1)){  # X3 (depends only on [imputed] X2) & y
        # G3[i+1,t] <- exp(-exp(loglam3[t-1]) + i*loglam3[t-1] - logfact(i))
        G3[i+1,1,t] <- exp(-exp(loglam3[t-2]) + i*loglam3[t-2] - logfact(i))
        G3[i+1,2,t] <- 0
        Q[i+1,1,t] <- sqrt(tauy)*exp(-0.5*tauy*pow((y[t] - (X2[t] + i + X4[t])),2))/sqrt(2*pi)
        Q[i+1,2,t] <- 0
      } 
      G3[(N_max3+1),1,t] <- max(0,1- sum(G3[1:(N_max3),1,t]))
      G3[(N_max3+1),2,t] <- 0
      Q[(N_max3+1),1,t] <- sqrt(tauy)*exp(-0.5*tauy*pow((y[t] - (X2[t] + N_max3 + X4[t])),2))/sqrt(2*pi)
      Q[(N_max3+1),2,t] <- 0        
      # for (j in 0:N_max3){ # X2 - constant rows (as depends only on [integrated] X1)
      #   P2[0+1,j+1,t] <- 0
      #   for (i in 1:(N_max1-1)){  # X2
      #     P2[i+1,j+1,t] <- exp(-exp(loglam2[i+1,t-1]) + X2[t]*loglam2[i+1,t-1] - logfact(X2[t]))
      #   }
      #   P2[(N_max1+1),j+1,t] <- max(0,1- sum(P2[(1:N_max1),j+1,t]))
      # }
      
      P2[0+1,1,t] <- 0
      P2[0+1,2,t] <- 0
      for (i in 1:(N_max1-1)){  # X2
        P2[i+1,1,t] <- exp(-exp(loglam2[i+1,t-1]) + X2[t]*loglam2[i+1,t-1] - logfact(X2[t]))
        P2[i+1,2,t] <- 0         
      }
      P2[(N_max1+1),1,t] <- max(0,1- sum(P2[(1:N_max1),1,t]))
      P2[(N_max1+1),2,t] <- 0
      
      for (i in 0:(N_max3)){  # X4 (depends on [integrated] X3)
        P4[i+1,1,t] <- ifelse((i + X4[t-1] - X4[t])>0,
