model{ 
  ...
  for(t in 3:T){ 
    ...
    G[(N_max+1),t] <- 0#max(0,1- sum(G[1:(N_max),t]))
    ...
    #                          0)
    P[(N_max+1),t] <-  exp(Na[t]*log(phia[t-1]) + (N_max + Na[t-1] - Na[t])*log(1-phia[t-1]) + logfact(N_max + Na[t-1]) - logfact(abs(N_max + Na[t-1] - Na[t])) - logfact(Na[t]))
    ...
    for (i in 0:(N_max-1)){  
      ...
      #                    0)
      P[i+1,t] <- exp(Na[t]*log(phia[t-1]) + (i + Na[t-1] - Na[t])*log(1-phia[t-1]) + logfact(i + Na[t-1]) - logfact(abs(i + Na[t-1] - Na[t])) - logfact(Na[t]))
    } 
