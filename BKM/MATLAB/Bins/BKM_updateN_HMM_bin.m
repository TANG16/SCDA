function [N,  accept, A_sum] = BKM_updateN_HMM_bin(fn_BKM_cov, N, theta, y, deltaN, priorN, bin, logfact)    
...
%             accept = accept+1;
    ...
    for t = 3:T    
        ...
        if (N(t) > 0)
            ...
            % Calculate the new likelihood value for the proposed move:
            % Calculate the numerator (num) and denominator (den) in turn:
            num = BKM_loglik_N_HMM_bin(y(t), N(t), N(t-1), N(t-2),...
                  phia(t-1), phi1(t-2), rho(t-2), sigy, bin, logfact);
            den = BKM_loglik_N_HMM_bin(y(t), Na_old, N(t-1), N(t-2),...
                  phia(t-1), phi1(t-2), rho(t-2), sigy, bin, logfact);
