...
%     fn_logfact = @(xx) sum(log(1:xx));   
    ...
    loglik_N1 = sum(log(poisspdf(N1(1:2),priorN(1)))) ...
        + sum(log(poisspdf(N1(3:T),lam)));
    loglik_Na = sum(log(binopdf(Na(1:2),priorN(2),priorN(3)))) ...
        + sum(log(binopdf(Na(3:T),N_tot(2:(T-1)),phia(2:(T-1)))));
