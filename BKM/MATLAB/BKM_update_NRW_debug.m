function [N, theta, accept, A_sum] = BKM_update_NRW_debug(N, theta, prior, delta, y, m, f, stdT, update_N, logfact)
% A normal random-walk (NRW) algorithm for the demographic regression parameters,
...
%         [N_new, accept, A_sum] = BKM_updateN_U_v2(fn_BKM_cov, N, theta, y, delta.N, prior.N, logfact);
%         [N_new, accept, A_sum] = BKM_updateN_U_debug(fn_BKM_cov, N, theta, y, delta.N, prior.N, logfact);
        [N_new, accept, A_sum] = BKM_updateN_debug(f,stdT, N, theta, y, delta.N, prior.N, logfact);
    else
    ...
    for ii = [1:3,5:7]
        ...
        % Propose a new value for the logistic regression parameter theta using a RW with normal proposal density
