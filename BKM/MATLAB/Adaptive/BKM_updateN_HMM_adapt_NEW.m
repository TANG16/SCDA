function [N,  accept] = BKM_updateN_HMM_adapt_NEW(fn_BKM_cov, N, theta, y,...
                        deltaN, priorN, mid, logfact)    
...
%                   logfact(Na_old + 1);      
            ...
            if (t == 2)  
                num = num + BKM_loglik_N_HMM_adapt_v2(N(t+1), N(t), priorN(1),...
                      phia(t), 1, 1, mid, logfact);
                den = den +  BKM_loglik_N_HMM_adapt_v2(N(t+1), Na_old, priorN(1),...
                      phia(t), 1, 1, mid, logfact);


                num = num + BKM_loglik_N_HMM_adapt_v2(N(t+2), N(t+1), N(t),...
                      phia(t+1), phi1(t), rho(t), mid, logfact);
                den = den +  BKM_loglik_N_HMM_adapt_v2(N(t+2), N(t+1), Na_old,...
                      phia(t+1), phi1(t), rho(t), mid, logfact);           
            end
...
%             accept = accept+1;
    ...
    for t = 3:T    
        ...
        if (N(t) > 0)
            ...
            % Calculate the numerator (num) and denominator (den) in turn:
           if (t == T)
                num = BKM_loglik_N_HMM_adapt_v2(N(t), N(t-1), N(t-2),...
                      phia(t-1), phi1(t-2), rho(t-2), mid, logfact) ... 
                      -  0.5*( ((y(t)-N(t)).^2)/sigy);
                den = BKM_loglik_N_HMM_adapt_v2(Na_old, N(t-1), N(t-2),...
                      phia(t-1), phi1(t-2), rho(t-2), mid, logfact) ...
                      -  0.5*(((y(t)-Na_old).^2)/sigy);   
                  
%                 num = num + BKM_loglik_N_HMM_adapt_v2(N(t+1), N(t), N(t-1),...
%                       phia(t), phi1(t-1), rho(t-1), mid, logfact);
%                 den = den +  BKM_loglik_N_HMM_adapt_v2(N(t+1), Na_old, N(t-1),...
%                       phia(t), phi1(t-1), rho(t-1), mid, logfact);
%                 num = num + BKM_loglik_N_HMM_EndCond(N(t-1), phi1(t-1), rho(t-1), mid, logfact);
%                 den = den + BKM_loglik_N_HMM_EndCond(N(t-1), phi1(t-1), rho(t-1), mid, logfact);

%                 num = num + BKM_loglik_N_HMM_adapt_v2(1, 0, N(t),...
%                       phia(t+1), phi1(t), rho(t), mid, logfact);
%                 den = den +  BKM_loglik_N_HMM_adapt_v2(N(t+2), N(t+1), Na_old,...
%                       phia(t+1), phi1(t), rho(t), mid, logfact);
% %                 num = num + BKM_loglik_N_HMM_EndCond(N(t), phi1(t), rho(t), mid, logfact);
% %                 den = den + BKM_loglik_N_HMM_EndCond(Na_old, phi1(t), rho(t), mid, logfact);
...
%                 den = den +  BKM_loglik_N_HMM_adapt_v2(N(t+2), N(t+1), Na_old,...
%                       phia(t+1), phi1(t), rho(t), mid, logfact);      
%                 num = num + BKM_loglik_N_HMM_adapt_EndCond(N(t), phi1(t), rho(t), mid, logfact);
%                 den = den + BKM_loglik_N_HMM_adapt_EndCond(Na_old, phi1(t), rho(t), mid, logfact);
            elseif (t == 3)
                num = BKM_loglik_N_HMM_adapt_v2(N(t), N(t-1), priorN(1),...
                      phia(t-1), 1, 1, mid, logfact) ... 
                      -  0.5*( ((y(t)-N(t)).^2)/sigy);
                den = BKM_loglik_N_HMM_adapt_v2(Na_old, N(t-1), priorN(1),...
                      phia(t-1), 1, 1, mid, logfact) ...
                      -  0.5*(((y(t)-Na_old).^2)/sigy); 
